...
abstract class UserStatsRepository {
  ...
  Future<void> endStudySession(String sessionId, DateTime endTime, int durationMinutes, int xpEarned);
  Future<List<StudySession>> getUserStudySessions(String userId, {DateTime? from, DateTime? to});
  Stream<List<StudySession>> getUserStudySessionsStream(String userId);
  
  // XP and Streak Management
  Future<void> addXp(String userId, int xp, {String? subjectId, String? reason});
  ...
  // Analytics
  Future<Map<String, dynamic>> getUserAnalytics(String userId, {DateTime? from, DateTime? to});
